---
description: mandatory checklist to run at the end of every phase or major implementation — always update skills and workflows before writing the walkthrough
---
...
Skills and workflows improve Antigravity's effectiveness on **future** tasks.
...
3. You are about to call `notify_user` to report the task complete

**The rule:** before every `notify_user` that closes an implementation task, ask:
> "Did I encounter anything I had to fix, retry, or look up during this task?"

If YES → run `/complete-phase` FIRST, then `notify_user`.
